...
| `chat` | [`initChat`](docs/classes/SDK.md#initchat) | AI chat interface. Bound to an AI agent when `agentId` is set, to the current user otherwise. Supports `fileId` and `threadId` to attach files or resume threads |
