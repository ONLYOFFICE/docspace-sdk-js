...
describe("login", () => {
  ...
  test("login() resolves with the portal's answer, including a two-factor challenge", async () => {
    const { inst, postMessageSpy } = initWithPostMessage();
    ...
    window.dispatchEvent(
      new MessageEvent("message", {
        data: JSON.stringify({
          ...
          methodReturnData: { tfa: true, confirmUrl: "https://portal.example.com/confirm/TfaAuth" },
          commandName: "login",
        }),
        origin: BASE_SRC,
      })
    ...
    await expect(promise).resolves.toEqual({ tfa: true, confirmUrl: "https://portal.example.com/confirm/TfaAuth" });
  });
});
